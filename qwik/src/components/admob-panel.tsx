...
export const AdMobPanel = component$(() => {
  ...
  const addLog = $((msg: string) => {
    ...
    state.logs = [`${new Date().toLocaleTimeString().split(' ')[0]} ${msg}`, ...state.logs].slice(0, 50);
  });
      ...
      AdMob.requestConsentInfo(
        ...
        () => {
          ...
          AdMob.initialize(() => {
            ...
            document.addEventListener('on.banner.load', () => addLog('Event: Banner Loaded'));
            document.addEventListener('on.interstitial.dismissed', () => addLog('Event: Interstitial Closed'));
            document.addEventListener('on.rewarded.earned', (e: any) => addLog(`Event: Reward ${e.amount}`));
            document.addEventListener('on.appopen.loaded', () => addLog(`Event: AppOpen Loaded: ${window.admobNextGen?.showAppOpenAd()}`));
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ 
        ...
        <h3 style={{ margin: '0 0 10px 0', color: '#666', fontSize: '14px', textTransform: 'uppercase' }}>SDK Status</h3>
      ...
      <div style={{
        ...
        {state.logs.map((log, i) => (
          <div key={i} style={{ borderBottom: '1px solid #1f2937', padding: '2px 0' }}>{log}</div>
        ))}
      </div>

      {/* CONTROLS GRID */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
        <button onClick$={showBanner} style={btnPrimary}>Show Banner</button>
        <button onClick$={removeBanner} style={btnDanger}>Hide Banner</button>
        
        <button onClick$={showInterstitial} style={{...btnWarning, gridColumn: 'span 2'}}>
            Show Interstitial
        </button>

        <button onClick$={showRewarded} style={{...btnPurple, gridColumn: 'span 2'}}>
            Show Rewarded
        </button>

        <button onClick$={showNativeTop} style={btnDark}>Native Top</button>
        <button onClick$={removeNative} style={btnSecondary}>Hide Native</button>

        <button onClick$={showAppOpen} style={{...btnSuccess, gridColumn: 'span 2'}}>
            Load App Open
        </button>
      </div>
    </div>
